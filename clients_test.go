...
func TestParseClientHistory(t *testing.T) {
	...
	clientHistory := []*unifi.ClientHistory{}
	...
	a.EqualValues(clientHistory[1].UnifiDevice, unifi.FlexBool{Val: true, Txt: "true"})
}
...
func TestClientHistoryOpts(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			actual: unifi.NewClientHistoryOpts(),
			expected: &unifi.ClientHistoryOpts{
		...
		{
			name:   "OnlyNonBlocked",
			actual: unifi.NewClientHistoryOpts().SetOnlyNonBlocked(true),
			expected: &unifi.ClientHistoryOpts{
				OnlyNonBlocked:      true,
				IncludeUnifiDevices: true,
				WithinHours:         0,
		...
		},
		{
			...
			actual: unifi.NewClientHistoryOpts().SetIncludeUnifiDevices(false),
			expected: &unifi.ClientHistoryOpts{
				OnlyNonBlocked:      false,
		...
		{
			name:   "WithinHours",
			actual: unifi.NewClientHistoryOpts().SetWithinHours(24),
			expected: &unifi.ClientHistoryOpts{
				OnlyNonBlocked:      false,
				IncludeUnifiDevices: true,
				WithinHours:         24,
